...
if(batch == TRUE){
  ...
  W <- res$W   # Undesired factors (2 columnas: W_1, W_2)
  ...
  if (method == "edgeR") {
    ...
    design <- model.matrix(~ W_1 + W_2+ condition, data = sample_list)
    ...
    res_filtered <- results$table[abs(results$table$logFC)>1 & results$table$FDR < 0.05,]
    ...
    res_filtered <- subset(results, abs(log2FoldChange) > 1 & padj < 0.05) |> as.data.frame()
...
###############################################################################
####                                                                       ####
####                  RepeatAnnotations Molecular Signature                ####
####                                                                       ####
###############################################################################
