...
data$condition <- relevel(data$condition, ref = positive_class)  


idx <- createDataPartition(data$condition, p = 0.8, list = FALSE)
train_df <- data[idx, ]
test_df  <- data[-idx, ]

# Check rápido
table(data$condition)
table(train_df$condition)
table(test_df$condition)

is_multi <- nlevels(train_df$condition) > 2
...
control <- trainControl(
  ...
  allowParallel = TRUE,
  sampling = NULL,  # <- CLAVE (o "up")
  summaryFunction = if (is_multi) multiClassSummary else twoClassSummary
)
...
rf_model <- train(
  ...
  data = train_df,
  method = "rf",
  metric = if (is_multi) "Accuracy" else "ROC",
  trControl = control
)
...
pred_rf$obs <- droplevels(pred_rf$obs)
...
if (length(clases_glm) == 2) {
  ...
  # 2) One vs all curves
  roc_list_glm <- lapply(clases_glm, function(cl) {
