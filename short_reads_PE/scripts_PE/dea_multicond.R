...
print(paste0(head(expression_matrix)))
...
###############################################################################
fdr_cutoff <- FDR_thr
lfc_cutoff <- log2FC_thr

contrast_files <- list.files(outdir, pattern = "^contrast_.*\\.csv$", full.names = TRUE)

results_list <- list()

for (f in contrast_files) {
  cname <- sub("^contrast_", "", sub("\\.csv$", "", basename(f)))
  df    <- read.csv(f, check.names = FALSE)
  
  if ("log2FoldChange" %in% names(df)) df$logFC <- df$log2FoldChange
  if ("padj"           %in% names(df)) df$FDR   <- df$padj
  
  df_filt <- df[!is.na(df$FDR) & !is.na(df$logFC) &
                  df$FDR < fdr_cutoff & abs(df$logFC) > lfc_cutoff, , drop = FALSE]
  
  if (nrow(df_filt) == 0) {
    message("Skipping (no DEGs): ", cname)
    next
  }
  
  results_list[[paste0("results_contrast_", cname)]] <- df_filt
}
...
for (nm in res_names) {
  results <- results_list[[nm]]
  cname   <- sub("^results_contrast_", "", nm)
  conds   <- strsplit(cname, "_vs_")[[1]]
...
ggsave(paste0(DEA_results_DIR,"/repetitive_counts_violin_box.png"), width = 8, 
       height = 6, dpi = 300)
ggsave(paste0(DEA_results_DIR,"/repetitive_counts_violin_box.pdf"), width = 8, 
       height = 6, dpi = 300)
