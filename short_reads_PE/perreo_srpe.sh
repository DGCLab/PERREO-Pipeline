...
perreo_banner "SR-PE" "Paired-end short-read sequencing technologies"
...
run_pipeline_sample() {
  ...
  perreo_summary "Pipeline parameters" \
    ...
    "Threads=${threads}" \
    "RAM=${ram} GB" \
    "R1 adaptor=${adapt_r1:-none}" \
    "R2 adaptor=${adapt_r2:-none}" \
    "Trimming=${trimming:-simple}" \
    "Quality threshold=${trimming_quality_threshold}" \
    "Minimum read length=${min_length_trim}" \
    "Initial trim R1=${initial_trim_read1}" \
    "Initial trim R2=${initial_trim_read2}" \
